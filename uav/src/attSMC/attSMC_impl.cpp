...
attSMC_impl::attSMC_impl(attSMC *self, const LayoutPosition *position, string name) {
  ...
  //YawRef=new DoubleSpinBox(reglages_groupbox->NewRow(),"YawRef_deg:",-360,360,0.01);
...
void attSMC_impl::UseDefaultPlot(const LayoutPosition *position) {
  GroupBox *Attitude_01 = new GroupBox(position,"Attitude Signals");

  GroupBox *EulerAngles = new GroupBox(Attitude_01->NewRow(),"Euler Angles");
  DataPlot1D *plotRoll = new DataPlot1D(EulerAngles->NewRow(),"Roll-Data",-50,50);
  plotRoll->AddCurve(state->Element(12), DataPlot::Red);
  plotRoll->AddCurve(state->Element(15), DataPlot::Black);

  DataPlot1D *plotPitch = new DataPlot1D(EulerAngles->LastRowLastCol(),"Pitch-Data",-50,50);
  plotPitch->AddCurve(state->Element(13), DataPlot::Red);
  plotPitch->AddCurve(state->Element(16), DataPlot::Black);

  DataPlot1D *plotYaw = new DataPlot1D(EulerAngles->LastRowLastCol(),"Yaw-Data",-50,50);
  plotYaw->AddCurve(state->Element(14),DataPlot::Red);
  plotYaw->AddCurve(state->Element(17),DataPlot::Black); 

  GroupBox *QuaternionError = new GroupBox(Attitude_01->NewRow(),"Quaternion Error");
  DataPlot1D *plotQe = new DataPlot1D(QuaternionError->NewRow(),"Quaternion - Error",-1.5,1.5);
  plotQe->AddCurve(state->Element(0),DataPlot::Red);
  plotQe->AddCurve(state->Element(1),DataPlot::Black);
  plotQe->AddCurve(state->Element(2),DataPlot::Blue);
  plotQe->AddCurve(state->Element(3),DataPlot::Green);
...
void attSMC_impl::UpdateFrom(const io_data *data) {
  ...
  q_yaw.w()=cos(deg2rad(YawRef->Value()/2));
  q_yaw.x()=0;
  q_yaw.y()=0;
  q_yaw.z()=sin(deg2rad(YawRef->Value()/2));

  Quaternionf q_ref = q_d*q_yaw;

  */

  //Generating the Quaternion error
  //Quaternionf q_e = q_ref.conjugate()*q;
  ...
  state->SetValueNoMutex(12, 0, ToDegree(Quad_Euler.roll));
  state->SetValueNoMutex(13, 0, ToDegree(Quad_Euler.pitch));
  state->SetValueNoMutex(14, 0, ToDegree(Quad_Euler.yaw));
  state->SetValueNoMutex(15, 0, ToDegree(Quad_Euler_d.roll));
  state->SetValueNoMutex(16, 0, ToDegree(Quad_Euler_d.pitch));
  state->SetValueNoMutex(17, 0, ToDegree(Quad_Euler_d.yaw));  
...
float attSMC_impl::deg2rad(float input_deg){
  return input_deg*3.1416/180;
}

float attSMC_impl::ToDegree(const float signal){
  return 180*signal/3.1416;
}
